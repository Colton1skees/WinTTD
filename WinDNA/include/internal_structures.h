...
// Reverse engineered WinDBG internal structures.
namespace Internals
{
	class Disx86
	{
	...
	public:
		...
		virtual u64* GetRegistersData(u64* outputDst);
		...
		virtual void GetRegister(RegisterId regId, u64* pDst, size_t regSize);
		virtual void SetRegister(RegisterId regId, u64* pSrc, size_t regSize);
		virtual u64 GetPC();
		virtual u64 GetTeb();
	...
	// Internal function for reading virual cpu memory.
	typedef bool(__fastcall* f_ReadCachedDataInternal)(VirtualCpu* vCpu, u64 addr, void* pDst, u64 dataSize);
	...
	class VirtualCpuHelper
	{
	private:
		inline static f_ReadCachedDataInternal readCachedDataInternal;
	public:
		static bool ReadVirtualCpuMemory(VirtualCpu* vCpu, u64 addr, void* pDst, u64 dataSize);
	};
}